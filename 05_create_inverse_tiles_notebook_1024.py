# %% [markdown]
# # 📦 Inverse Design Tiles Generation - 1024×1024 Version
#
# Forward 데이터를 역순으로 사용하여 Inverse Design용 타일을 생성합니다.
#
# **데이터 방향:**
# - Input: EM Intensity Map (1024×1024 .npy) ← Forward의 outputs
# - Output: Pillar Pattern (1024×1024 .png) ← Forward의 inputs
#
# ## 📋 목차
# 1. 환경 설정 및 임포트
# 2. 파라미터 설정
...
# %%
# ==================== 타일 생성 파라미터 ====================
# 메모리 절약 전략: 1024×1024 샘플 40개 (5 μm × 5 μm)
DATA_DIR = 'data/forward_intensity'          # Forward 데이터 디렉토리
OUTPUT_DIR = 'data/inverse_tiles_1024'       # Inverse 타일 출력 디렉토리
TILE_SIZE = 256                          # 타일 크기
NUM_TILES_PER_SAMPLE = 250               # 샘플당 타일 개수 (1024×1024이므로 감소)
TRAIN_SAMPLES = 32                       # 훈련용 샘플 개수 (40개 중 32개)
VAL_SAMPLES = 8                          # 검증용 샘플 개수 (40개 중 8개)
...
print("✅ 파라미터 설정 완료!")
print(f"\n📊 Inverse Tiles 생성 정보 (1024×1024 버전):")
...
print(f"\n💡 전략: 작은 샘플(1024×1024)을 많이(40개) 사용하여 메모리 절약")
...
if len(all_samples) > 0:
    ...
    if sample_phase.shape[0] == 1024 and sample_phase.shape[1] == 1024:
...
# - `07_inverse_design_notebook_1024.py`: Inverse 예측 (원하는 phase → pillar 설계)
