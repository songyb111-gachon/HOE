...
def createInputOutputLostLists(inputHeight, inputWidth, inputs, lastDeconvTask1, lastDeconvTask2, lastDeconvTask3,
                               lastDeconvTask4, lastDeconvTask5, lastDeconvTask6, lastDeconvTask7, outputChannelNos, outputTypes):
    inputList = []
    outputList = []
    lossList = []
    ...
    if taskNo >= 6:
        outputWeights6 = Input(shape = (inputHeight, inputWidth))
        [output6, loss6] = outputLayer(lastDeconvTask6, outputWeights6, outputTypes[5], outputChannelNos[5], 'out6')
        inputList.append(outputWeights6)
        outputList.append(output6)
        lossList.append(loss6)
    if taskNo >= 7:
        outputWeights7 = Input(shape = (inputHeight, inputWidth))
        [output7, loss7] = outputLayer(lastDeconvTask7, outputWeights7, outputTypes[6], outputChannelNos[6], 'out7')
        inputList.append(outputWeights7)
        outputList.append(output7)
        lossList.append(loss7)    
...
def unet(inputHeight, inputWidth, channelNo, outputChannelNos, outputTypes, layerNum, noOfFeatures, 
    ...
    if layerNum == 1:
        ...
        lastDeconvTask6 = oneDecoderPath(lastConv, layerNum, noOfFeatures, filterSize, dropoutRate, conv1, conv2, conv3, conv4)
        lastDeconvTask7 = oneDecoderPath(lastConv, layerNum, noOfFeatures, filterSize, dropoutRate, conv1, conv2, conv3, conv4)
    elif layerNum == 5:
    ...
    [inputList, outputList, lossList] = createInputOutputLostLists(inputHeight, inputWidth, inputs, lastDeconvTask1, 
                                                                   ...
                                                                   lastDeconvTask5, lastDeconvTask6, lastDeconvTask7, outputChannelNos, outputTypes)   
