...
def unetSingleRegression(inputHeight, inputWidth, layerNum, noOfFeatures, dropoutRate):
    ...
    return model
############################################################################################################
def unetMultiRegression(inputHeight, inputWidth, layerNum, noOfFeatures, dropoutRate):
    outputChannelNo = 6
    inputChannelNo = 1
    ...
    model.compile(loss = lossList, optimizer = optimizer, metrics = ['mean_squared_error'])
    return model
############################################################################################################
def cascaded(inputHeight, inputWidth, layerNum, noOfFeatures, dropoutRate, taskWeights):
    outputChannelNo = 6
    inputChannelNo = 1
