# %% [markdown]
# # 🎓 Inverse Design U-Net 모델 학습 - 1024×1024 Version
#
# 256×256 타일로 Inverse Design U-Net 모델을 학습합니다.
# (1024×1024 샘플에서 추출한 타일 사용)
#
# **모델:**
...
# %%
# ==================== 데이터 파라미터 ====================
DATA_PATH = 'data/inverse_tiles_1024/train'  # 1024×1024 버전 타일
...
LOG_DIR = 'logs'
EXPERIMENT_NAME = 'inverse_design_basic_tiles_1024'  # 1024 버전 실험명
...
print(f"   Device: {device}")
print(f"\n💡 전략: 작은 샘플(1024×1024)에서 추출한 타일로 효율적 학습")
...
train_dataset = InverseDesignDataset(
    data_path='data/inverse_tiles_1024/train',
    input_extension='npy',
...
val_dataset = InverseDesignDataset(
    data_path='data/inverse_tiles_1024/val',
    input_extension='npy',
    output_extension='png',
    normalize=True  # Min-Max 정규화로 [0, 1] 범위로 변환
)
...
#
# **다음 단계:**
# - `07_inverse_design_notebook_1024.py`: 원하는 phase map으로부터 pillar pattern 설계
...
print(f"\n🚀 다음 단계: 07_inverse_design_notebook_1024.py 실행!")
